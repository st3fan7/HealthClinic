...
namespace Repository.Csv.Converter
{
    public class SurgeryCSVConverter : ICSVConverter<Surgery>
    {
        ...
        public Surgery ConvertCSVFormatToEntity(string entityCSVFormat)
        {
            ...
            return new Surgery(int.Parse(tokens[0]), bool.Parse(tokens[1]), tokens[2], new Room(int.Parse(tokens[3])), (Model.Doctor.DoctorSpecialist)new User(int.Parse(tokens[4])), 
                (Patient)new User(int.Parse(tokens[5])), DateTime.Parse(tokens[6]), DateTime.Parse(tokens[7]));
        }
